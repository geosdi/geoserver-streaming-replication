 ...
 */
public class CoverageStoreInfoReceiverOperation extends AbstractReceiverOperation<CoverageStoreInfo> {

    public CoverageStoreInfoReceiverOperation(DependencyBag dependencyBag) {
        super(dependencyBag);
    }

    @Override
    void executeModifyOperation(CoverageStoreInfo catalogInfo) {
        LOGGER.log(Level.INFO, "Adding ... {0}", catalogInfo.getId());
        CoverageStoreInfo theOriginal = super.dependencyBag.getCatalog().getCoverageStore(catalogInfo.getId());
        super.dependencyBag.getMapper().map(catalogInfo, theOriginal);
        super.dependencyBag.getCatalog().save(theOriginal);
    }

    @Override
    void executeAddOperation(CoverageStoreInfo resourceInfo) {
        LOGGER.log(Level.INFO, "Adding ... {0}", resourceInfo.getId());
        super.dependencyBag.getCatalog().add(resourceInfo);
    }

    @Override
    void executeRemoveOperation(CoverageStoreInfo catalogInfo) {
