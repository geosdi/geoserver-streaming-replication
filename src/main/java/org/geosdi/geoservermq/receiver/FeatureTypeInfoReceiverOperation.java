 ...
 */
public class FeatureTypeInfoReceiverOperation extends AbstractReceiverOperation<FeatureTypeInfo> {

    public FeatureTypeInfoReceiverOperation(DependencyBag dependencyBag) {
        super(dependencyBag);
    }

    @Override
    void executeModifyOperation(FeatureTypeInfo catalogInfo) {
        LOGGER.log(Level.INFO, "Adding ... {0}", catalogInfo.getId());
        FeatureTypeInfo theOriginal = super.dependencyBag.getCatalog().getFeatureType(catalogInfo.getId());
        super.dependencyBag.getMapper().map(catalogInfo, theOriginal);
        super.dependencyBag.getCatalog().save(theOriginal);
    }

    @Override
    void executeAddOperation(FeatureTypeInfo catalogInfo) {
        LOGGER.log(Level.INFO, "Adding ... {0}", catalogInfo.getId());
        super.dependencyBag.getCatalog().add(catalogInfo);
    }

    @Override
    void executeRemoveOperation(FeatureTypeInfo catalogInfo) {
        LOGGER.log(Level.INFO, "Removing ... {0}", catalogInfo.getName());
        ResourceInfo resource = super.dependencyBag.getCatalog().getResource(
                catalogInfo.getId(), ResourceInfo.class);
        CascadeDeleteVisitor visitor = new CascadeDeleteVisitor(super.dependencyBag.getCatalog());
        catalogInfo.accept(visitor);
        if (resource != null) {
            List<LayerInfo> selection = super.dependencyBag.getCatalog().getLayers(
                    resource);
            if (selection != null) {
