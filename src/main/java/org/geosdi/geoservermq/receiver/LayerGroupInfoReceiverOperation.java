 ...
 */
public class LayerGroupInfoReceiverOperation extends AbstractReceiverOperation<LayerGroupInfo> {

    public LayerGroupInfoReceiverOperation(DependencyBag dependencyBag) {
        super(dependencyBag);
    }

    @Override
    void executeModifyOperation(LayerGroupInfo catalogInfo) {
        LOGGER.log(Level.INFO, "Adding ... {0}", catalogInfo.getId());
        LayerGroupInfo theOriginal = super.dependencyBag.getCatalog().getLayerGroup(catalogInfo.getId());
        super.dependencyBag.getMapper().map(catalogInfo, theOriginal);
        super.dependencyBag.getCatalog().save(theOriginal);
    }

    @Override
    void executeAddOperation(LayerGroupInfo catalogInfo) {
        LOGGER.log(Level.INFO, "Adding ... {0}", catalogInfo.getId());
        super.dependencyBag.getCatalog().add(catalogInfo);
    }

    @Override
    void executeRemoveOperation(LayerGroupInfo catalogInfo) {
