 ...
 */
public class LayerInfoReceiverOperation extends AbstractReceiverOperation<LayerInfo> {

    public LayerInfoReceiverOperation(DependencyBag dependencyBag) {
        super(dependencyBag);
    }

    @Override
    void executeModifyOperation(LayerInfo catalogInfo) {
        LOGGER.log(Level.INFO, "Adding ... {0}", catalogInfo.getId());
        LayerInfo theOriginal = super.dependencyBag.getCatalog().getLayer(catalogInfo.getId());
        super.dependencyBag.getMapper().map(catalogInfo, theOriginal);
        super.dependencyBag.getCatalog().save(theOriginal);
    }

    @Override
    void executeAddOperation(LayerInfo catalogInfo) {
        LOGGER.log(Level.INFO, "Adding ... {0}", catalogInfo.getId());
        super.dependencyBag.getCatalog().add(catalogInfo);
    }

    @Override
    void executeRemoveOperation(LayerInfo catalogInfo) {
