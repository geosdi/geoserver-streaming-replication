 ...
 */
public class DataStoreInfoReceiverOperation extends AbstractReceiverOperation<DataStoreInfo> {

    public DataStoreInfoReceiverOperation(DependencyBag dependencyBag) {
        super(dependencyBag);
    }

    @Override
    void executeModifyOperation(DataStoreInfo catalogInfo) {
        LOGGER.log(Level.INFO, "Adding ... {0}", catalogInfo.getId());
        DataStoreInfo theOriginal = super.dependencyBag.getCatalog().getDataStore(catalogInfo.getId());
        super.dependencyBag.getMapper().map(catalogInfo, theOriginal);
        super.dependencyBag.getCatalog().save(theOriginal);
    }

    @Override
    void executeAddOperation(DataStoreInfo catalogInfo) {
        LOGGER.log(Level.INFO, "Adding ... {0}", catalogInfo.getId());
        super.dependencyBag.getCatalog().add(catalogInfo);
    }

    @Override
    void executeRemoveOperation(DataStoreInfo catalogInfo) {
