 ...
 */
public class WorkspaceInfoReceiverOperation extends AbstractReceiverOperation<WorkspaceInfo> {

    public WorkspaceInfoReceiverOperation(DependencyBag dependencyBag) {
        super(dependencyBag);
    }

    @Override
    void executeModifyOperation(WorkspaceInfo catalogInfo) {
        LOGGER.log(Level.INFO, "Adding ... {0}", catalogInfo.getId());
        WorkspaceInfo theOriginal = super.dependencyBag.getCatalog().getWorkspace(catalogInfo.getId());
        super.dependencyBag.getMapper().map(catalogInfo, theOriginal);
        super.dependencyBag.getCatalog().save(theOriginal);
    }

    @Override
    void executeAddOperation(WorkspaceInfo catalogInfo) {
        LOGGER.log(Level.INFO, "Adding ... {0}", catalogInfo.getId());
        super.dependencyBag.getCatalog().add(catalogInfo);
    }

    @Override
    void executeRemoveOperation(WorkspaceInfo catalogInfo) {
